...
const Part3DeploymentModels: React.FC<Part3DeploymentModelsProps> = ({ onComplete }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-8 bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-700 animate-fade-in">
      <h2 className="text-2xl font-bold text-cyan-400 mb-2">{t('part3.title')}</h2>
      <p className="text-slate-400 mb-6">{t('part3.subtitle')}</p>
      
      <div className="bg-slate-900/50 p-6 rounded-lg mb-6">
        <p className="text-lg text-slate-300 italic">"{t(question.scenarioKey)}"</p>
      </div>
      ...
      <div className="space-y-3">
        {question.options.map((option, index) => (
          <button
            ...
            className={`w-full text-left p-4 rounded-lg font-semibold transition-all duration-200 disabled:opacity-80 ${
          ...
          >
            {t(`deploymentModels.${option}` as any)}
          </button>
        ))}
      </div>
      ...
      {showFeedback && (
        <div className={`mt-6 p-4 rounded-lg text-white animate-fade-in ${isCorrect ? 'bg-green-500/20 border-green-500' : 'bg-red-500/20 border-red-500'} border`}>
          <h3 className="font-bold text-lg">{isCorrect ? t('part3.correctFeedbackTitle') : t('part3.incorrectFeedbackTitle')}</h3>
          ...
          <button onClick={handleNext} className="mt-4 w-full bg-gradient-to-r from-cyan-500 to-violet-600 text-white font-bold py-2 px-4 rounded-full hover:scale-105 transform transition-transform">
            {currentQuestionIndex < DEPLOYMENT_MODEL_QUESTIONS.length - 1 ? t('part3.nextButton') : t('part3.finishButton')}
          </button>
        </div>
      )}
    </div>
